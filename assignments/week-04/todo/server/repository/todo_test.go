...
func (s *ToDoRepositorySuite) TestDelete() {
	item := &pb.Todo{Id: "new_item", Title: "meeting"}
	errInsert := s.todoRep.Insert(item)
	s.Nil(errInsert)

	errDelete := s.todoRep.Delete(item.Id)
	s.Nil(errDelete)

	newTodo, getErr := s.todoRep.Get(item.Id)
	s.Equal("pg: no rows in result set", getErr.Error())
	var nilTodo *pb.Todo
	s.Equal(nilTodo, newTodo)
}
